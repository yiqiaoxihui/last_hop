...
local function icmp_pu_listener(send_l3_sock,signal,ip,iface)
	...
	while signal['status']==0 do
		...
		if status then
			...
			print(ip,l3_icmp_pu_packet.ip_src,l3_icmp_pu_packet.ip_dst,"icmp pu")
...
local function guest_network_distance_by_traceroute(end_ttl,error_ttl,iface,send_l3_sock,ip,icmp_echo_listener_signal,icmp_tole_listener_signal)
	local times=0
	local time_limit_ttl=0
	local echo_reply_ttl=0
	...
	if time_limit_ttl+1==echo_reply_ttl then
		...
		print(ip,echo_reply_ttl,"traceroute fail get max time limit")
		return -1
	end
...
function guest_network_distance(iface,send_l3_sock,icmp_echo_listener_signal,icmp_tole_listener_signal,ip)
	...
	if icmp_echo_listener_signal['receive']==true then
		...
		-- min_ttl=ttl_from_target_to_source-3
		-- max_ttl=ttl_from_target_to_source+5
		if min_ttl<1 then
			min_ttl=1
		end
		if max_ttl>35 then
	...
	local time_limit_ttl=0		--max time limit ttl
	local echo_reply_ttl=0		--min echo reply ttl
	while true do
