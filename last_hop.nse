...
---
-- @usage
-- sudo nmap --script last_hop -iL ip1  -sn
-- 
-- @output
-- | get last hop: 
...
--用于接受icmp生存时间过期报文
--从中提取末跳路由器信息
local function icmp_tole_listener()
	...
	local status,len,l2_icmp_t_l,l3_icmp_tol,time=icmp_tole_rec_socket:pcap_receive()
...
local function icmp_pu_listener(send_l3_sock)
	...
	local status,len,l2_icmp_pu_data,l3_icmp_pu_data,time=icmp_pu_rec_socket:pcap_receive()
...
local icmp_tole_listener_handler=stdnse.new_thread(icmp_tole_listener)
--建立监听线程
--用于接受icmp端口不可达包
--
-- @param icmp_pu_listener function name
-- @param send_l3_sock l3 layer raw socket
local icmp_pu_listener_handler=stdnse.new_thread(icmp_pu_listener,send_l3_sock)

stdnse.sleep(1)
--建立基于udp的socket
--用于发送udp大端口报文
local send_udp_socket=nmap.new_socket("udp")
print(host.ip,":send udp packet, port:65534")
...
repeat
	if coroutine.status(icmp_pu_listener_handler)=="dead" then
		...
		print("wait for icmp port unreachable packet...")
		stdnse.sleep(3)
	end
...
repeat
	if coroutine.status(icmp_tole_listener_handler)=="dead" then
		...
		print("wait icmp time to live exceeded packet...")
		stdnse.sleep(3)
	end
